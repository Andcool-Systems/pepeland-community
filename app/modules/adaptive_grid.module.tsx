...
const AdaptiveGrid = ({ child_width, children, header, className, max_columns }: AdaptiveGridProps) => {
    ...
    useEffect(() => {
        const resizeObserver = new ResizeObserver(entries => {
            ...
            setColumnCount(Math.min(Math.max(1, Math.floor(width / (child_width + 15))), max_columns ?? Infinity));
        });
